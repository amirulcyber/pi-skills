...
| Default identity | `pi-agent` | `pi-agent` — one identity on both hosts (owner decision 2026-09-25), email `323469836+amirulcyber@users.noreply.github.com` (the account's registered noreply form — GitHub rejects any other, and an unregistered one leaves commits unlinked). Export `GIT_AUTHOR_*`/`GIT_COMMITTER_*` before sourcing to override. Attribution only; auth is the bundled `amirulcyber` key on both. |
...
(`pi-agent <323469836+amirulcyber@users.noreply.github.com>`):
